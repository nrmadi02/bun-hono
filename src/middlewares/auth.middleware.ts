  ...
  next: Next
	...
	try {
		...
		if(!findToken.userId || payload.id !== findToken.userId) {
			return errorResponse(c, "Unauthorized - Token does not belong to this user", ["Unauthorized - Token does not belong to this user"], 401);
		}
...
export const validateRefreshToken = createMiddleware<{
  ...
  const authHeader = c.req.header("Authorization");
	if (!authHeader || !authHeader.startsWith("Bearer ")) {
		return errorResponse(c, "Unauthorized - No refresh token provided", ["Unauthorized - No refresh token provided"], 401);
	}
	...
	const isRefreshTokenExpired = refreshTokenExpiresAt && refreshTokenExpiresAt < new Date();

	if (!refreshToken) {
		return errorResponse(c, "Unauthorized - Invalid refresh token", ["Unauthorized - Invalid refresh token"], 401);
	}

	if (isRefreshTokenExpired) {
		return errorResponse(c, "Unauthorized - Refresh token expired", ["Unauthorized - Refresh token expired"], 401);
	}
	...
	try {
		...
		if(!findToken.userId || payload.id !== findToken.userId) {
			return errorResponse(c, "Unauthorized - Refresh token does not belong to this user", ["Unauthorized - Refresh token does not belong to this user"], 401);
		}
